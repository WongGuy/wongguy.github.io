...
- `screw-selector.html`, `nut-selector.html`, `washer-selector.html`, `torque-calcs.html`, `preload-calcs.html` — one page per reference tool. Each has its own `<section class="chart-card">` and an empty `<div class="site-nav"></div>` that `tab-nav.js` fills with the shared two-level nav. `torque-calcs.html` and `preload-calcs.html` are placeholders with no tool built yet.
...
- `flowchart.js` — shared, page-agnostic decision-tree flowchart renderer. `renderFlowchart(container, data, options)` takes a plain JSON tree (see the doc comment at the top of the file for the Node/Edge shape) and builds it as real DOM boxes with CSS-drawn connector lines, so it reflows and wraps on resize instead of breaking a fixed-width layout. Each page that shows a flowchart gets its own `<tool>-flowchart-data.js` holding just the JSON tree (plus any page-specific wiring, e.g. a detail toggle) and an empty `<div id="flowchart" class="flowchart"></div>` — see `nut-flowchart-data.js` (uses `options.activeTags` to toggle fine-pitch variants) and `washer-flowchart-data.js` (no toggle) for two different shapes of tree reusing the same renderer.
...
Each reference tool follows the same pattern: a data file (e.g. `screw-data.js`) describing the values, plus rendering logic (e.g. `screw-app.js`) that builds the table/panel from it. When adding a new reference tool, give it its own `<tool>-selector.html`, `<tool>-data.js`, `<tool>-app.js`, `assets/images/<tool>/`, and `assets/standards/ISO/<tool>/`, following this same data/render split rather than hardcoding values into HTML. Add the page to `site-nav-data.js` so it shows up in the nav.
